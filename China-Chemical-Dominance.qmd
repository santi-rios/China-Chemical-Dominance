...
ui <- fluidPage(
  ...
  div(class = "card",
      div(class = "card-header", "Analytical Views"),
      div(class = "card-body",
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    # By default, we select "United States" and "China" if present
    default_sel <- c()
    if ("United States" %in% valid_countries) default_sel <- c(default_sel, "United States")
    if ("China" %in% valid_countries)         default_sel <- c(default_sel, "China")
  ...
  output$combinedPlot <- renderPlotly({
    ...
    gg_base <- ggplot(data_subset, aes(x = Year, color = Country)) +
      geom_line(aes(y = Value.x), linewidth = 1, na.rm = TRUE) +
  ...
  ###########################################
  ## 6) Cartogram image if "Figure1-a"    ##
  ###########################################
