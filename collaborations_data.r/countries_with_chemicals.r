...
df_fig1 <- df %>%
  ...
  # filter(!Country %in% c("US w/o China", "USA alone", "CN alone", "China w/o US"))

unique(df_fig1$Country)

countries_with_chemicals_joined <- df_fig1 |> 
  left_join(chemichal_countries, by =c( "Country", "Year", "iso3c"))

str(countries_with_chemicals_joined)
write.csv(countries_with_chemicals_joined, "./data/countries_with_chemicals_joined.csv")
...
## Example: df must have columns:
##   Country, Year, Value.x, source.x, iso3c, Value.y, source.y, substance
## Here, assume you've already loaded df in your environment, e.g.:
## df <- countries_with_chemicals_joined
## For demonstration, just reassign df (omitted real data)
# df <- some_prepared_data_frame
df <- countries_with_chemicals_joined %>% 
  mutate(Country = as.factor(Country))
## Convert columns to factor or numeric as needed
## e.g. df$Country <- as.factor(df$Country)
